...
#include "wiener/WienerLinienActivity.h"
...
void ActivityManager::renderTaskLoop() {
  while (true) {
    ...
    if (currentActivity) {
      ...
      // This appliance owns its board polarity through the Dark/Light setting.
...
void ActivityManager::goToWienerLinien() {
  auto activity = makeUniqueNoThrow<WienerLinienActivity>(renderer, mappedInput);
  if (!activity) {
    LOG_ERR("ACT", "OOM: Wiener Linien activity");
    return;
  }
  replaceActivity(std::move(activity));
}
